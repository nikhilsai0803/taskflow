    ...
    <div style={{ minHeight:'100vh', background:'#0f1117', display:'flex', alignItems:'center', justifyContent:'center', padding:20, fontFamily:"'DM Sans', sans-serif" }}>
      <div style={{ background:'#161b27', border:'1px solid #2a3550', borderRadius:20, padding:40, width:'100%', maxWidth:400 }}>
        <div style={{ textAlign:'center', marginBottom:28 }}>
          ...
          <h2 style={{ fontSize:22, fontWeight:700, color:'#e8edf8' }}>TaskFlow</h2>
          <p style={{ fontSize:13, color:'#8a99be', marginTop:4 }}>Sign in to your workspace</p>
        </div>

        <form onSubmit={submit}>
          <div style={{ marginBottom:14 }}>
            <label style={lbl}>Email</label>
            <input style={inp} type="email" value={form.email} onChange={set('email')} placeholder="you@company.co" required />
          </div>
        ...
        <p style={{ textAlign:'center', marginTop:20, fontSize:13, color:'#8a99be' }}>
          No account? <Link to="/register" style={{ color:'#4f8ef7' }}>Sign up</Link>
        </p>
        <div style={{ marginTop:16, padding:12, background:'#1e2638', borderRadius:10, fontSize:12, color:'#8a99be', lineHeight:1.6 }}>
          💡 Seed an admin via <code style={{color:'#4f8ef7'}}>/api/auth/register</code> with <code style={{color:'#4f8ef7'}}>role: "admin"</code>
        </div>
      </div>
    </div>
  );
